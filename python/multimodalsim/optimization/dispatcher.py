...
class ShuttleGreedyDispatcher(Dispatcher):
    ...
    def dispatch(self, state):
        ...
        # non_assigned_vehicles = [veh for veh in state.vehicles if veh.route.status == VehicleStatus.RELEASE]
        ...
        all_vehicles_sorted_by_departure_time = sorted(all_vehicles, key=lambda x: x.route.current_stop.departure_time)

        vehicles_sorted_by_departure_time = non_assigned_vehicles_sorted_by_departure_time
        for req in non_assigned_requests:

            if len(vehicles_sorted_by_departure_time) == 0:
                vehicles_sorted_by_departure_time = all_vehicles_sorted_by_departure_time.copy()

            # The passenger must be ready before the departure time.
            potential_vehicles = list(x for x in vehicles_sorted_by_departure_time
                                      if x.route.current_stop.departure_time > req.ready_time)

            logger.debug(
                "potential_vehicles={}".format(list(veh.id for veh in potential_vehicles)))
            ...
            vehicles_sorted_by_departure_time = [x for x in vehicles_sorted_by_departure_time
            ...
            for node in path:
                ...
                if distance > 0:
                    ...
                    logger.debug("{}: id(stop)={} | {} {} {}".format(assigned_vehicle.id, id(stop), arrival_time,
                                                                     departure_time, location))
                    next_stops.append(stop)
                    previous_node = node

            if len(assigned_vehicle.route.next_stops) != 0 and len(next_stops) != 0 \
                    and assigned_vehicle.route.next_stops[-1].location.get_coordinates() == \
...
class FixedLineDispatcher(Dispatcher):
    ...
    def __find_optimal_vehicle_for_leg(self, leg):
        ...
        for vehicle in self.__state.vehicles:
            ...
            if origin_departure_time is not None and origin_departure_time > self.__state.current_time \
                    and origin_departure_time >= leg.trip.ready_time and destination_arrival_time is not None \
                    and (earliest_arrival_time is None or destination_arrival_time < earliest_arrival_time):
