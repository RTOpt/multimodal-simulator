...
class VehicleBoarding(Event):
    ...
    def process(self, env):
        ...
        if len(self.__route.requests_to_pickup()) > 0:
            # Passengers to board
            passengers_to_board_copy = self.__route.current_stop.passengers_to_board.copy()
            for req in passengers_to_board_copy:
                self.__route.current_stop.initiate_boarding(req)
                passenger_event_process.PassengerToBoard(req, self.queue).add_to_queue()
...
class VehicleArrival(Event):
    ...
    def process(self, env):
        ...
        passengers_to_alight_copy = self.__route.current_stop.passengers_to_alight.copy()
        for trip in passengers_to_alight_copy:
            if trip.current_leg in self.__route.onboard_legs:
                self.__route.alight(trip)
                passenger_event_process.PassengerAlighting(trip, self.queue).add_to_queue()
