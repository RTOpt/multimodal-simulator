...
class Optimize(Event):
    def __init__(self, time, queue):
        super().__init__('Optimize', queue, time, state_machine=queue.env.optimization.state_machine)
    ...
    def process(self, env):
        ...
        state.fix_routes_for_time_interval(env.optimization.fixed_time_interval)

        optimization_result = env.optimization.dispatch(state)

        state.unfix_routes_for_time_interval(env.optimization.fixed_time_interval)
...
class EnvironmentUpdate(Event):
    def __init__(self, optimization_result, queue):
        super().__init__('EnvironmentUpdate', queue, state_machine=queue.env.optimization.state_machine)
    ...
    def process(self, env):
        ...
        for trip in self.__optimization_result.modified_requests:
            ...
            passenger_update = request.PassengerUpdate(trip.current_leg.assigned_vehicle.id, trip.id, current_leg,
                                                       next_legs)
...
class EnvironmentIdle(Event):
    def __init__(self, queue):
        super().__init__('EnvironmentIdle', queue, state_machine=queue.env.optimization.state_machine)
