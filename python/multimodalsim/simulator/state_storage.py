...
class StateStorage:
    ...
    def env(self) -> Optional['environment_module.Environment']:
        raise NotImplementedError('env of {} not implemented'.
                                  format(self.__class__.__name__))
    ...
    def queue(self) -> Optional['event_queue_module.EventQueue']:
        raise NotImplementedError('queue of {} not implemented'.
                                  format(self.__class__.__name__))

    @queue.setter
    def queue(self, events: Optional[list['event_queue_module.EventQueue']]) \
            -> None:
        raise NotImplementedError('queue of {} not implemented'.
                                  format(self.__class__.__name__))
...
class StateStorageJSON(StateStorage):
    ...
    @property
    def queue(self) -> Optional[list['event_queue_module.EventQueue']]:
        return self.__queue

    @queue.setter
    def queue(self, queue: Optional[list['event_queue_module.EventQueue']]) \
            -> None:
        self.__queue = queue

    def save_state(self):
        env_copy = self.__env.get_environment_copy()
        queue_copy = self.__queue.get_queue_copy()
    ...
    def load_state(self, state_file_name: str):
        ...
        self.__env = simulation_state.env
        self.__queue = simulation_state.queue
